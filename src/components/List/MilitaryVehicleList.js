...
import {URL_MILITARY_VEHICLE,} from "../../services/PrivateServices";
import axios from "axios";
import Cookies from "js-cookie";

const MilitaryVehicleList = () => {
    const [militaryVehicles, setMilitaryVehicles] = useState([]);

    const fetchMilitaryVehicles = async () => {
        try {
            const token = Cookies.get('authToken');
            if (!token) return;

            const { data } = await axios.get(URL_MILITARY_VEHICLE, {
                headers: { Authorization: `Bearer ${token}` }
            });

            setMilitaryVehicles(data.data);
        } catch (err) {}
    };


    useEffect(() => {
        fetchMilitaryVehicles();
    }, []);

    const sortedVehicles = Array.isArray(militaryVehicles)
        ? militaryVehicles.sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
        : [];
...
export default MilitaryVehicleList;
